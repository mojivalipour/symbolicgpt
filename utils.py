...
def layer_norm(input_tensor, name=None, epsilon=1e-5):
    ...
    with tf.compat.v1.variable_scope(name2use, default_name='LayerNorm'):
        ...
        mean = tf.compat.v1.reduce_mean(input_tensor, axis=-1, keepdims=True)
        std = tf.compat.v1.reduce_mean(tf.square(input_tensor - mean), axis=-1, keepdims=True)
        input_tensor = (input_tensor - mean) * tf.compat.v1.rsqrt(std + epsilon)
...
def construct_scalar_host_call(metric_dict, model_dir, prefix=""):
    ...
    def host_call_fn(global_step, *args):
        ...
        with tf.summary.create_file_writer(
                logdir=model_dir, filename_suffix=".host_call").as_default():
            #with tf.summary.always_record_summaries(): # this is always on in the new version
            for i, name in enumerate(metric_names):
...
# load libraries and other requirements
import six

    ...
    def __init__(self,
        ...
        with tf.variable_scope(scope, default_name='newslm', reuse=reuse):
            with tf.variable_scope("embeddings"):
                ...
                self.seq_length = embeddings.shape[1] if embeddings.shape[1]!=None else self.seq_length+1
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if is_training:
            train_op, train_metrics = optimization_adafactor.create_optimizer(
                total_loss, learning_rate, num_train_steps, num_warmup_steps, use_tpu, gradient_accumulation_multiplier=gradient_accumulation)
...
def classification_model_fn_builder(config: GroverConfig, init_checkpoint, learning_rate,
                                    ...
                                    adafactor=False, adam_bfloat=False, lm_loss_coef=0.5, gradient_accumulation=20):

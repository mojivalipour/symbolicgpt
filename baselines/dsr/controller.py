...
class LinearWrapper(tf.compat.v1.nn.rnn_cell.BasicRNNCell): #tf.contrib.rnn.LayerRNNCell):
    """
...
class Controller(object):
    ...
    def __init__(self, sess, prior, debug=0, summary=True,
        ...
        with tf.compat.v1.name_scope("controller"):
            ...
            if embedding:
                with tf.compat.v1.variable_scope("embeddings",
                    ...
                    if observe_action:
                        action_embeddings = tf.compat.v1.get_variable("action_embeddings", [n_action_inputs, embedding_size], trainable=True)
                    if observe_parent:
                        parent_embeddings = tf.compat.v1.get_variable("parent_embeddings", [n_parent_inputs, embedding_size], trainable=True)
                    if observe_sibling:
                        sibling_embeddings = tf.compat.v1.get_variable("sibling_embeddings", [n_sibling_inputs, embedding_size], trainable=True)
            ...
            def get_input(obs):
                ...
                if observe_action:
                    if embedding:
                        obs = tf.nn.embedding_lookup(params=action_embeddings, ids=action)
                    else:
                        obs = tf.one_hot(action, depth=n_action_inputs)
                    observations.append(obs)
                if observe_parent:
                    if embedding:
                        obs = tf.nn.embedding_lookup(params=parent_embeddings, ids=parent)
                    else:
                        obs = tf.one_hot(parent, depth=n_parent_inputs)
                    observations.append(obs)
                if observe_sibling:
                    if embedding:
                        obs = tf.nn.embedding_lookup(params=sibling_embeddings, ids=sibling)
                    else:
            ...
            def get_next_obs_prior_dangling(actions_ta, dangling):
                ...
                actions = tf.transpose(a=actions_ta.stack()) # Shape: (?, time)
                
                # Compute parent, sibling, prior, and dangling
                action, parent, sibling, prior, dangling = tf.compat.v1.py_func(func=get_action_parent_sibling_prior_dangling,
            ...
            self.obs = [tf.transpose(a=obs_ta.stack(), perm=[1, 0]) for obs_ta in obs_tas] # [(?, max_length)] * 3
        ...
        def make_batch_ph(name):
            with tf.compat.v1.name_scope(name):
                batch_ph = {
                    ...
                    "obs" : (tf.compat.v1.placeholder(tf.int32, [None, max_length]),
                             tf.compat.v1.placeholder(tf.int32, [None, max_length]),
                             tf.compat.v1.placeholder(tf.int32, [None, max_length])),
                    "priors" : tf.compat.v1.placeholder(tf.float32, [None, max_length, n_choices]),
        ...
        def make_neglogp_and_entropy(B):
            ...
            entropy = tf.reduce_sum(input_tensor=entropy_per_step * mask, axis=1) # Sum over time dim -> (batch_size, )   
        ...
        with tf.compat.v1.name_scope("losses"):
            ...
            # Entropy loss
            entropy_loss = -self.entropy_weight * tf.reduce_mean(input_tensor=entropy, name="entropy_loss")
            loss = entropy_loss
            ...
            if ppo:
                ...
                self.old_neglogp_ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=(None,), name="old_neglogp")
                ratio = tf.exp(self.old_neglogp_ph - neglogp)
                clipped_ratio = tf.clip_by_value(ratio, 1. - ppo_clip_ratio, 1. + ppo_clip_ratio)
                ppo_loss = -tf.reduce_mean(input_tensor=tf.minimum(ratio * (r - self.baseline), clipped_ratio * (r - self.baseline)))
                loss += ppo_loss
                ...
                self.sample_kl = tf.reduce_mean(input_tensor=neglogp - self.old_neglogp_ph)

            # Policy gradient loss
            else:
                if not pqt or (pqt and pqt_use_pg):
                    pg_loss = tf.reduce_mean(input_tensor=(r - self.baseline) * neglogp, name="pg_loss")                    
                    loss += pg_loss

            # Priority queue training loss
            if pqt:
                pqt_neglogp, _ = make_neglogp_and_entropy(self.pqt_batch_ph)
                pqt_loss = pqt_weight * tf.reduce_mean(input_tensor=pqt_neglogp, name="pqt_loss")
                loss += pqt_loss
        ...
        with tf.compat.v1.name_scope("summary"):
            if self.summary:
                if ppo:
                    tf.compat.v1.summary.scalar("ppo_loss", ppo_loss)
                else:
                    if not pqt or (pqt and pqt_use_pg):
                ...
                tf.compat.v1.summary.scalar("reward", tf.reduce_mean(input_tensor=r))
